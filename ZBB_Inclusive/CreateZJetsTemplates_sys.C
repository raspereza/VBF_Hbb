...
void CreateSignalPDF(int iCAT,
  ...
  for (unsigned int i=0; i<5; ++i) {
    ...
    treeZJets[sysName]->Draw("mbb>>"+nameHist,"weight*("+cuts[iCAT]+")");
    mapNorm[sysName] = hist->GetSumOfWeights();
    delete hist;

    hist = new TH1D(nameHist,"",NbinsSig,xmin,xmax);
    treeZJets[sysName]->Draw("mbb>>"+nameHist,"weight");
...
void CreateZJetsTemplates_sys() {
  ...
  TFile * fileOutput = new TFile("rootshape/zj_doubleb_shapes.root","recreate");
