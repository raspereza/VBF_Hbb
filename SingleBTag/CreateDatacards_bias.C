...
void CreateDatacards_bias(bool TF = false) {
  ...
  for (int i=0; i<5; ++i) {
    ...
    ostringstream str;
    str << "datacards_bias/datacards_singleb_" << names[i] << ".txt";
    string nn = str.str();
    ...
    textfile << "shapes          qqH_hbb_bias    *   SingleBTag/root_shape/signal_singleb_shapes.root   w:qqH_$CHANNEL" << endl;
    textfile << "shapes          ggH_hbb_bias    *   SingleBTag/root_shape/signal_singleb_shapes.root   w:ggH_$CHANNEL" << endl;
