...
void CreateSignalPDF(int iCAT,
  ...
  for (unsigned int i=0; i<9; ++i) {
    ...
    treeZJets[sysName]->Draw(variable+">>"+nameHist,"weight*1.0*("+cuts[iCAT]+")");
    treeZJets_ewk[sysName]->Draw(variable+">>"+nameHist_ewk,"weight*1.0*("+cuts[iCAT]+")");
    ...
    treeZJets[sysName]->Draw(variable+">>"+nameHist,"weight*1.0");
    treeZJets_ewk[sysName]->Draw(variable+">>"+nameHist_ewk,"weight*1.0");    
    hist->Add(hist_ewk); //addition of EWK ZJets contribution with QCD ZJets
...
void CreateZJetscombinedTemplates_sys_full_2() {
  ...
  TFile * fileOutput = new TFile("root_shape/zj_singleb_shapes_2016.root","recreate");
  fileOutput->cd("");
