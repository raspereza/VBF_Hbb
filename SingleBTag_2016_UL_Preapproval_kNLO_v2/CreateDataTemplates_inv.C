...
void CreatePDF(int iCAT, 
  ...
  // inverse 
  RooRealVar a1("a1_"+names[iCAT],"a1",-5.0,5.0);
  RooRealVar a2("a2_"+names[iCAT],"a2",-5.0,5.0);
  RooRealVar a3("a3_"+names[iCAT],"a3",-5.0,5.0);
  RooRealVar a4("a4_"+names[iCAT],"a4",-5.0,5.0);
  RooRealVar a5("a5_"+names[iCAT],"a5",-5.0,5.0);
  ...
  RooGenericPdf QCD((string("qcd_")+namesCAT[iCAT]).c_str(),genericInvPower.at(iORDER).c_str(),RooArgSet(mbb,argList));
...
void CreateDataTemplates_inv() {
  ...
  TFile * fileOutput = new TFile("root_shape/data_singleb_shapes_2016.root","recreate");
  fileOutput->cd("");
  RooWorkspace * w = new RooWorkspace("w","data");

  //  int iorder[6] = {2,2,2,2,1,1};
  int iorder[4] = {2,2,2,2}; 
  for (int i=1; i<4; ++i) 
