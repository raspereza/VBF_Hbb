...
void Fits(int iCAT = 4, 
  ...
  TNtuple * treeZewk = (TNtuple*)file->Get("Mass_and_BDT_ZJets_EWK");
  ...
  TString nameHistZewk = "mbb_zj_ewk";
  TString nameHistZ_norm = "mbb_zj_norm";
  TString nameHistZewk_norm = "mbb_zj_ewk_norm";

  TString nameHistVBF = "mbb_qqH";
  TString nameHistGGH = "mbb_ggh";
  TString nameHistVBF_norm = "mbb_qqH_norm";
  TString nameHistGGH_norm = "mbb_ggh_norm";
  ...
  TH1D * histZewk = new TH1D(nameHistZewk,"",NbinsSig,xmin_mc,xmax_mc);
  TH1D * histZ_norm = new TH1D(nameHistZ_norm,"",NbinsSig,xmin_mc,xmax_mc);
  TH1D * histZewk_norm = new TH1D(nameHistZewk_norm,"",NbinsSig,xmin_mc,xmax_mc);

  TH1D * histVBF = new TH1D(nameHistVBF,"",NbinsSig,xmin_mc,xmax_mc);
  TH1D * histGGH = new TH1D(nameHistGGH,"",NbinsSig,xmin_mc,xmax_mc);
  TH1D * histVBF_norm = new TH1D(nameHistVBF_norm,"",NbinsSig,xmin_mc,xmax_mc);
  TH1D * histGGH_norm = new TH1D(nameHistGGH_norm,"",NbinsSig,xmin_mc,xmax_mc);

  treeVBF->Draw(variable+">>"+nameHistVBF,"weight");
  treeGGH->Draw(variable+">>"+nameHistGGH,"weight");
  TH1D * histH = (TH1D*)histVBF->Clone("histH");
  histH->Add(histH,histGGH,1.,1.);

  treeVBF->Draw(variable+">>"+nameHistVBF_norm,"weight*("+Cuts+")");
  treeGGH->Draw(variable+">>"+nameHistGGH_norm,"weight*("+Cuts+")");

  double normVBF = histVBF_norm->GetSumOfWeights();
  double normGGH = histGGH_norm->GetSumOfWeights();

  treeZ->Draw(variable+">>"+nameHistZ,"weight");
  treeZewk->Draw(variable+">>"+nameHistZewk,"weight");
  histZ->Add(histZ,histZewk, 1.,1.);

  treeZ->Draw(variable+">>"+nameHistZ_norm,"weight*("+Cuts+")");
  treeZewk->Draw(variable+">>"+nameHistZewk_norm,"weight*("+Cuts+")");
  
  double normZ = histZ_norm->GetSumOfWeights();
  double normZewk = histZewk_norm->GetSumOfWeights();

  tree->Draw(variableData+">>"+nameHist,Cuts);
  tree->Draw(variableData+">>"+nameBlindHist,CutsBlind);
  double yield_ini = hist->GetSumOfWeights();
  double z_yield = normZ+normZewk;
  ...
  RooBernstein QCD("qcd_"+names[iCAT],"qcd_"+names[iCAT],mbb,argList);
  ...
  cout << endl;
  cout << "Signal shape fit ---> " << endl;
  cout << "mean(sig)  = " << mean_sig.getVal() << " +/- " << mean_sig.getError() << endl;
  cout << "sigma(sig) = " << sigma_sig.getVal() << " +/- " << sigma_sig.getError() << endl;
  cout << "alpha(sig) = " << alpha_sig.getVal() << " +/- " << alpha_sig.getError() << endl;
  cout << "n(sig)     = " << n_sig.getVal() << " +/- " << n_sig.getError() << endl;
  cout << "frac(sig)  = " << f_sig.getVal() << " +/- " << f_sig.getError() << endl;
  cout << endl;
  cout << "Z->bb shape fit ---> " << endl;
  cout << "mean(zbb)  = " << mean_zj.getVal() << " +/- " << mean_zj.getError() << endl;
  cout << "sigma(zbb) = " << sigma_zj.getVal() << " +/- " << sigma_zj.getError() << endl;
  cout << "alpha(zbb) = " << alpha_zj.getVal() << " +/- " << alpha_zj.getError() << endl;
  cout << "n(zbb)     = " << n_zj.getVal() << " +/- " << n_zj.getError() << endl;
  cout << "frac(zbb)  = " << f_zj.getVal() << " +/- " << f_zj.getError() << endl;
  cout << endl;
  ...
  cout << "r(Z)             = " << xZbb/z_yield << " +/- " << errZbb/z_yield << std::endl;  
  cout << endl;
  cout << "NormVBF          = " << normVBF << endl;
  cout << "NormGGH          = " << normGGH << endl;
  cout << "NormZ_QCD        = " << normZ << endl;
  cout << "NormZ_EWK        = " << normZewk << endl;
  cout << "NormZ            = " << z_yield << endl;
