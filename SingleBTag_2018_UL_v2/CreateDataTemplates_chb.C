...
void CreatePDF (int iCAT,
  ...
  tree->Draw(variable+">>"+nameHist,cuts[iCAT]);
  tree_zj->Draw(variable+">>"+nameHist_zj,"weight*("+cuts[iCAT]+")");
  tree_vbf->Draw(variable+">>"+nameHist_vbf,"weight*("+cuts[iCAT]+")");
  tree_ggh->Draw(variable+">>"+nameHist_ggh,"weight*("+cuts[iCAT]+")");

  TH1D * histData = (TH1D*)hist->Clone("dataHist_"+names[iCAT]);

  hist->Add(hist_zj,-znorm[iCAT]);
  hist->Add(hist_vbf,-1);
  hist->Add(hist_ggh,-1);
  ...
  RooExponential CHV("qcd_"+names[iCAT],"qcd_"+names[iCAT],);
