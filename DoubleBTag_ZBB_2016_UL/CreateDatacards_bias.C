...
void CreateDatacards_bias(bool TF = false) {
  ...
  for (int i=0; i<4; ++i) {
    ...
    ostringstream str;
    str << "datacards/datacards_doubleb_" << names[i] << "_09_12_2021.txt";
    string nn = str.str();
    ...
    textfile << "kmax *" << endl;
    textfile << "----------------------------------------------------------------------------------------------------" << endl;
    textfile << "shapes          qcd             *   DoubleBTag_2016_UL/root_shape/data_doubleb_shapes.root       w:qcd_$CHANNEL" << endl;
    textfile << "shapes          data_obs        *   DoubleBTag_2016_UL/root_shape/data_doubleb_shapes.root       w:data_$CHANNEL" << endl;
    textfile << "shapes          qqH_hbb         *   DoubleBTag_2016_UL/root_shape/signal_doubleb_shapes.root   w:qqH_$CHANNEL" << endl;
    textfile << "shapes          ggH_hbb         *   DoubleBTag_2016_UL/root_shape/signal_doubleb_shapes.root   w:ggH_$CHANNEL" << endl;
    textfile << "shapes          qqH_hbb_bias    *   DoubleBTag_2016_UL/root_shape/signal_doubleb_shapes.root   w:qqH_$CHANNEL" << endl;
    textfile << "shapes          ggH_hbb_bias    *   DoubleBTag_2016_UL/root_shape/signal_doubleb_shapes.root   w:ggH_$CHANNEL" << endl;
    textfile << "shapes          tt              *   DoubleBTag_2016_UL/root_shape/tt_doubleb_shapes.root  w:tt_$CHANNEL" << endl;
    textfile << "shapes          wj              *   DoubleBTag_2016_UL/root_shape/wj_doubleb_shapes.root  w:wj_$CHANNEL" << endl;
    textfile << "shapes          zj              *   DoubleBTag_2016_UL/root_shape/zj_doubleb_shapes.root  w:zj_$CHANNEL" << endl;
    textfile << "----------------------------------------------------------------------------------------------------" << endl;
    ...
    //QGL-reweighting
    textfile << fixed << setprecision(3)<< "CMS_QGLweight_2016  lnN     " << (1 + ggh_qgl->GetBinContent(i+1)) <<  "  " << (1 + vbfh_qgl->GetBinContent(i+1)) << "        -       " <<   (1 + tt_qgl->GetBinContent(i+1)) << "      " << (1 + wj_qgl->GetBinContent(i+1))  << "   " << (1 + zj_qgl->GetBinContent(i+1))  << "   "  << (1 + ggh_qgl->GetBinContent(i+1)) <<  "  " << (1 + vbfh_qgl->GetBinContent(i+1)) << endl;
    //PS ISR
