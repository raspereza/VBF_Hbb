...
#include "Common.h"
void PlotDataFitsQCD(int iCAT = 1, 
		     int iORDER = 6,
		     TString ZWeight = "1.0*",
		     bool blind = true,
		     bool BERN = false) {
  ...
  TString cutsBlind;
  cutsBlind = cuts[iCAT] + "&&(mbb_reg<70||mbb_reg>120)";
  ...
  zjmc->Draw("mbb_reg>>"+nameHist_zj,ZWeight+"weight*("+cuts[iCAT]+")");
  ...
  zjmc->Draw("mbb_reg>>"+nameBlindHist_zj,ZWeight+"weight*("+cutsBlind+")");
  ttmc->Draw("mbb_reg>>"+nameBlindHist_tt,"weight*("+cutsBlind+")");
  stmc->Draw("mbb_reg>>"+nameBlindHist_st,"weight*("+cutsBlind+")");

  delete dummy;
  
  hist->Add(hist,hist_wj,1.,-1.);
  hist->Add(hist,hist_zj,1.,-1.);
  hist->Add(hist,hist_tt,1.,-1.);
  hist->Add(hist,hist_st,1.,-1.);

  histBlind->Add(histBlind,histBlind_wj,1.,-1.);
  histBlind->Add(histBlind,histBlind_zj,1.,-1.);
  histBlind->Add(histBlind,histBlind_tt,1.,-1.);
  histBlind->Add(histBlind,histBlind_st,1.,-1.);
 
  InitData(hist);

  int nBins = hist->GetNbinsX();
  
  std::cout << "Cuts  : " << cuts[iCAT] << std::endl;
  std::cout << "Data  : " << hist->GetSumOfWeights() << std::endl;
  std::cout << "ZJets : " << hist_zj->GetSumOfWeights() << std::endl;
  std::cout << "TTBar : " << hist_tt->GetSumOfWeights() << std::endl;
  std::cout << "WJets : " << hist_wj->GetSumOfWeights() << std::endl;
 ...
 if(BERN)
  ...
  char str[20];
  int cat_n = iCAT;
  sprintf(str,"DoubleB CAT %d",cat_n);
