...
void CreateDatacards_bias(bool freeZnorm = true,
  ...
  TFile  *fs = new TFile("VBFHbb_2016_SingleB_Systematics.root");
  ...
  //NLO ZJets k-factors
  TFile *f = new TFile("VBFHBB_2016_ZJets_NLO_kfact_syst_unc.root");
  ...
  for (int i=1; i<4; ++i) {
    ...
    textfile << "shapes          wj              *   " << folder << "workspace/wj_singleb_shapes"+suffix+".root  w:wj_$CHANNEL" << endl;
    textfile << "shapes          tt              *   " << folder << "workspace/top_singleb_shapes"+suffix+".root  w:tt_$CHANNEL" << endl;
    textfile << "----------------------------------------------------------------------------------------------------" << endl;
    ...
    textfile << "process          ggH_hbb   qqH_hbb   qcd   zj   ggH_hbb_bias  qqH_hbb_bias   wj  tt" << endl;
    ...
    textfile << "lumi_13TeV_corr   lnN  1.020    1.020    -    1.020   1.020    1.020   1.020   1.020" << endl;
    textfile << "lumi_13TeV_2016   lnN  1.010    1.010    -    1.010   1.010    1.010   1.010   1.010" << endl;
    textfile << "----------------------------------------------------------------------------------------------------" << endl;
    //Offline BTAG
    ...
    //    Dipole
    textfile << fixed << setprecision(3)<< "VBF_Dipole_Recoil   lnN     " << "  -       " <<  dipole_sysval[i] << "   -   -   -  " <<  dipole_sysval[i] << "   -   -" << endl;
    //L1 Prefire
    textfile << fixed << setprecision(3)<< "CMS_L1Prefire_2016  lnN     " << (1 + ggh_l1pf_up->GetBinContent(i)) << "/" <<  (1 + ggh_l1pf_dn->GetBinContent(i)) << "  " << (1 + vbfh_l1pf_up->GetBinContent(i)) << "/" <<  (1 + vbfh_l1pf_dn->GetBinContent(i))    << "        -       " << (1 + zj_l1pf_up->GetBinContent(i)) << "/" <<  (1 + zj_l1pf_dn->GetBinContent(i)) << "   " << (1 + ggh_l1pf_up->GetBinContent(i)) << "/" <<  (1 + ggh_l1pf_dn->GetBinContent(i)) << "  " << (1 + vbfh_l1pf_up->GetBinContent(i)) << "/" <<  (1 + vbfh_l1pf_dn->GetBinContent(i))  << "   -   -" << endl;
    //NLO k-factor
    textfile << "----------------------------------------------------------------------------------------------------" << endl;
    ...
    textfile << "normW_" << names[i] << " rateParam " << names[i] << " wj 1 [-2,2]" << endl;
    textfile << "normW_" << names[i] << "  param 1.0 0.3" << endl;
    textfile << "normTop_" << names[i] << " rateParam " << names[i] << " tt 1 [-2,2]" << endl;
    textfile << "normTop_" << names[i] << "  param 1.0 0.3" << endl;
