...
void CreateSignalPDF(int iCAT,
  ...
  for (unsigned int i=0; i<9; ++i) {
    ...
    treeGGH[sysName]->Draw(variable+">>"+nameGGHHist,"weight*1.0*("+cuts[iCAT]+")");
    treeVBF[sysName]->Draw(variable+">>"+nameVBFHist,"weight*1.0*("+cuts[iCAT]+")");
    ...
    treeGGH[sysName]->Draw(variable+">>"+nameGGHHist,"weight*1.0");
    treeVBF[sysName]->Draw(variable+">>"+nameVBFHist,"weight*1.0");
  ...
  double d_Mean2 = mapMean["ScaleUp"] - Mean2;
  ...
  double normGGH_mean_SCL = 0.5*(mapNormGGH["ScaleUp"]+mapNormGGH["ScaleDown"]);
  ...
  double normGGH_shift_SCL = (mapNormGGH["ScaleUp"]/normGGH_mean_SCL -1.0)*normGGH_Central;
  ...
  double normVBF_mean_SCL = 0.5*(mapNormVBF["ScaleUp"]+mapNormVBF["ScaleDown"]);
  ...
  double normVBF_shift_SCL = (mapNormVBF["ScaleUp"]/normVBF_mean_SCL -1.0)*normVBF_Central;
  ...
  outtext << "SCLUp   (ggH)  = " << mapNormGGH["ScaleUp"] << endl;
  outtext << "SCLDown (ggH)  = " << mapNormGGH["ScaleDown"] << endl;
  ...
  outtext << "SCLUp   (qqH)  = " << mapNormVBF["ScaleUp"] << endl;
  outtext << "SCLDown (qqH)  = " << mapNormVBF["ScaleDown"] << endl;
...
void CreateSignalTemplates_sys_full_2() {
  ...
  TFile * fileOutput = new TFile("root_shape/signal_singleb_shapes_2016.root","recreate");
  fileOutput->cd("");
