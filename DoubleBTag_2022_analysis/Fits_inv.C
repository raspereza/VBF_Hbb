...
#include "Common.h"
//root -l -b -q -e 'gROOT->ProcessLine(".L Fits_inv.C"); RunAllFits(true);'
void Fits_inv(int iCAT=0, 
	  int iORDER=1,
	  bool blind=true,
  ...
  std::cout << "Data events: ********************: " <<  hist->Integral() <<  std::endl;
  /*
  ...
  */
  ...
  RooGenericPdf QCD(polynomNames.at(iORDER).c_str(),genericPolynoms.at(iORDER).c_str(),RooArgSet(mbb,argListPol));
 ...
 double chi2_ = 0.0;
  ...
  char str3[30];
  sprintf(str3,"inv pol%1i+Zpeak",iORDER);   
  ...
  text->Draw();
   TLatex *tex_cms = new TLatex(0.57,0.94,"#bf{CMS} #it{Preliminary}                         35.0 fb^{-1} (2022)  ");
   tex_cms->SetNDC();
...
leg->AddEntry(xframe->findObject("qcd_curve"), "QCD", "L");
  ...
  */
  ...
  canv->Update();
  //TString dir = "/eos/user/m/mukherje/www/VBFHBB/2022/bkg_fit/";
  //TString dir = "plot_for_slides/";
  TString dir = "/eos/home-s/sapradha/www/VBFHbb_Run3/NeuralNetwork/DNN/Fits/inv";
  canv->Print(dir+"/2022_data_inv_pol_fit_cat_"+names[iCAT]+"_"+blinded+".png");
  canv->Print(dir+"/2022_data_inv_pol_fit_cat_"+names[iCAT]+"_"+blinded+".pdf");
}
...
void RunAllFits(bool blind=true, double ymin_ratio=0.69, double ymax_ratio=1.31)
{
  ...
  int iorder[nCAT] = {1,1,1,1,1,1,1};

  // If you prefer a mixed choice, e.g. {0,0,0,0,2,2,2}:
  // int iorder[nCAT] = {0,0,0,0,2,2,2};
  ...
  for (int icat = 0; icat < nCAT; ++icat) {
    ...
    Fits_inv(icat, iorder[icat], blind, ymin_ratio, ymax_ratio);
  }
}
