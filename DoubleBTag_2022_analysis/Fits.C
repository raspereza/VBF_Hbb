...
#include "Common.h"
//root -l -b -q -e 'gROOT->ProcessLine(".L Fits.C"); RunAllFits(true);'
void Fits(int iCAT=2, 
	  int iORDER=0,
	  bool blind=true,
  ...
  */
  ...
  RooRealVar tau_decay("tau_decay_"+names[iCAT],"tau_decay",-1e-1, -1e-6);
...
leg->AddEntry(xframe->findObject("qcd_curve"), "QCD", "L");
  ...
  */
  ...
  TString dir = "/eos/home-s/sapradha/www/VBFHbb_Run3/NeuralNetwork/DNN/Fits/exp";
  canv->Print(dir+"/2022_data_exp_pol_fit_cat_"+names[iCAT]+"_"+blinded+".png");
  canv->Print(dir+"/2022_data_exp_pol_fit_cat_"+names[iCAT]+"_"+blinded+".pdf");
}
...
void RunAllFits(bool blind=true, double ymin_ratio=0.69, double ymax_ratio=1.31)
{
  ...
  for (int icat = 0; icat < nCAT; ++icat) {
    ...
    Fits(icat, iorder[icat], blind, ymin_ratio, ymax_ratio);
  }
}
