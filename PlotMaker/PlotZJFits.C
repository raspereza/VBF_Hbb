...
void PlotZJFits(int iCAT=0) {
  ...
  TNtuple * treeVBF = (TNtuple*)file->Get("Mass_and_BDT_ZJets");
  ...
  for (int iB=1; iB<120; ++iB)
{
  ...
  RooRealVar mean("mean_sig_"+names[iCAT],"Mean",90,80,200);
  RooRealVar meanScale("CMS_mbb_scale","Mean",90,80,200);
  ...
  leg->AddEntry(histVBF1,"Z+jets","f");
  ...
  text->Draw();
  canv->Update();
  canv->Print("Plots/png/2016_UL_ZJ_singleb_combined_Mbb_model.png");
  canv->Print("Plots/pdf/2016_UL_ZJ_singleb_combined_Mbb_model.pdf");
  }
