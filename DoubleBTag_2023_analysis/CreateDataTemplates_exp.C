...
void CreatePDF(int iCAT, 
  ...
  treeZ->Draw(variable+">>"+nameHistZ,"T_weight*LUMI*T_HLTweight*T_PUweight*T_V_pTweight_QCD*T_V_pTweight_EWK*T_btag_weight_central*T_online_btag_weight");
//  treeZ->Draw(variable+">>"+nameHistZ, selection);
...
void CreateDataTemplates_exp() {
	...
	TFile* file_data = new TFile(dirName + "/tree_tree_data_2023_full_btg_sf.root");
TFile* file_zjet = new TFile(dirName + "/tree_Zto2Q-4Jets_TuneCP5_13p6TeV_madgraphMLM-pythia8_2023_btgsf.root");
TFile* file_vbfh = new TFile(dirName + "/tree_VBFHto2B_M-125_TuneCP5_13p6TeV_powheg-pythia8_2023_btgsf.root");
TFile* file_ggh  = new TFile(dirName + "/tree_GluGluHto2B_M-125_TuneCP5_13p6TeV_powheg-minlo-pythia8_2023_btgsf.root");
  ...
  TFile * fileOutput = new TFile("workspace/data_resolved_vbfhbb_2023.root","recreate");
  fileOutput->cd("");
  RooWorkspace * w = new RooWorkspace("w","data");

  int iorder[7] = {1,0,0,0,2,2,2}; 
