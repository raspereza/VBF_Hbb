...
void CreateSignalPDF(int iCAT,
  ...
  for (unsigned int i=0; i<9; ++i) {
    ...
    treeZJets[sysName]->Draw(variable+">>"+nameHist,"T_weight*LUMI*T_HLTweight*T_PUweight*T_V_pTweight_QCD*T_V_pTweight_EWK*T_btag_weight_central*T_online_btag_weight*("+cuts[iCAT]+")");
    //treeZJets_ewk[sysName]->Draw(variable+">>"+nameHist_ewk,"T_weight*LUMI*T_HLTweight*T_PUweight*T_V_pTweight_QCD*T_V_pTweight_EWK*( 1 + "+ dipole[iCAT] + ") * ("+cuts[iCAT]+")");
    //hist->Add(hist_ewk); //addition of EWK ZJets contribution with QCD ZJets
    hist->Scale(luminosity);
    hist_ewk->Scale(luminosity);
    ...
    treeZJets[sysName]->Draw(variable+">>"+nameHist,"T_weight*LUMI*T_HLTweight*T_PUweight*T_V_pTweight_QCD*T_V_pTweight_EWK*T_btag_weight_central*T_online_btag_weight");
    //treeZJets_ewk[sysName]->Draw(variable+">>"+nameHist_ewk,"T_weight*T_HLTweight*T_PUweight*T_V_pTweight_QCD*T_V_pTweight_EWK");    
    //hist->Add(hist_ewk); //addition of EWK ZJets contribution with QCD ZJets
    hist->Scale(luminosity);
    hist_ewk->Scale(luminosity);
    ...
    RooRealVar alphax("alpha","Alpha",0.5,0.,2);
  ...
  RooRealVar alpha("alpha_zj_"+names[iCAT],"alpha",0.5,0.,2.);
  RooRealVar n("n_zj_"+names[iCAT],"n",10,0,20);
  RooRealVar fsig("f_zj_"+names[iCAT],"fsig",0,1);
  ...
  double norm_mean_SCL = 0.5*(mapNorm["PNetJECUp"]+mapNorm["PNetJECDown"]);
  ...
  outtext << "SCLUp     = " << mapNorm["PNetJECUp"] << endl;
  outtext << "SCLDown   = " << mapNorm["PNetJECDown"] << endl;
...
void CreateZJetscombinedTemplates_sys_full_2() {
  ...
  for (int i=0; i<9; ++i) {
    TFile * file = new TFile(dirName+"/"+FileNamesZJetDNN[i]);
    TString sysName = sysNames[i];
    treeZJets[sysName] = (TNtuple*)file->Get("tree");
    //treeZJets_ewk[sysName] = (TNtuple*)file->Get("tree");
  }

  TFile * fileOutput = new TFile("workspace/zj_resolved_vbfhbb_2023.root","recreate");
  fileOutput->cd("");
  RooWorkspace * w = new RooWorkspace("w","signal");

  RooRealVar theta_JES("CMS_JES_2023","CMS_JES_2023",0.,-5.,5.);
  RooRealVar theta_JER("CMS_JER_2023","CMS_JER_2023",0.,-5.,5.);
  RooRealVar theta_SCL("CMS_scale_b_2023","CMS_scale_b_2023",0.,-5.,5.);
  RooRealVar theta_SMR("CMS_res_b_2023","CMS_res_b_2023",0.,-5.,5.);

  for (int i=0; i<7; ++i) 
  {
    //CreateSignalPDF(i,treeZJets,treeZJets_ewk,theta_JES,theta_JER,theta_SCL,theta_SMR,w);
    CreateSignalPDF(i,treeZJets,theta_JES,theta_JER,theta_SCL,theta_SMR,w);
  }
  w->Write("w");
  fileOutput->Write();
  fileOutput->Close();
