...
void Numbers(TString folder = "DoubleBTag_2016_UL_Preapproval_kNLO_v2") {
  ...
  std::map<TString, std::vector<TString> > samples = {
    ...
    {"QCD",{"Mass_and_BDT_QCD"}},
    {"Data",{"Mass_and_BDT_DATA"}}
  };

  TFile * file = new TFile(folder+"/ntuples/mbb_and_bdt_all_Nom.root");
  for (auto sample : samples) {
    ...
    for (auto sampleName : sampleNames) {
      ...
      if (name=="Data")
	tuple->Draw(variableData+">>"+histName);
      else if (name=="W"||name=="Top")
	tuple->Draw(variableData+">>"+histName,"weight");
      else
	tuple->Draw(variable+">>"+histName,"weight");
