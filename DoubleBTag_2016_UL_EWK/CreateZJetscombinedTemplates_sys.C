...
std::ofstream outtext("zj_combined_out.txt");
...
void CreateSignalPDF(int iCAT,
  ...
  for (unsigned int i=0; i<5; ++i) {
    ...
    TString nameHist_ewk = "mbb_zj_ewk";
  ...
  norm_shift_JES = 0.1*norm_Central;
  norm_shift_JER = 0.02*norm_Central;
...
void CreateZJetscombinedTemplates_sys() {
  ...
  for (int i=0; i<6; ++i) 
    CreateSignalPDF(i,treeZJets,treeZJets_ewk,theta_JES,theta_JER,w);
