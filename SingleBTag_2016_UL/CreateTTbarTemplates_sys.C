...
void CreateSignalPDF(int iCAT,
  ...
  for (unsigned int i=0; i<5; ++i) {
    ...
    TH1D * hist = new TH1D(nameHist,"",NbinsSig_tt,xmin_tt,xmax_tt);    
    treeTTbar[sysName]->Draw("mbb_reg>>"+nameHist,"weight*("+cuts[iCAT]+")");
    mapNorm[sysName] = hist->GetSumOfWeights();
    delete hist;

    hist = new TH1D(nameHist,"",NbinsSig_tt,xmin_tt,xmax_tt);
    treeTTbar[sysName]->Draw("mbb_reg>>"+nameHist,"weight");

    RooRealVar mbbx("mbb","mass(bb)",xmin_tt,xmax_tt);
  ...
  RooRealVar mbb("mbb_"+names[iCAT],"mass(bb)",xmin_tt,xmax_tt);
