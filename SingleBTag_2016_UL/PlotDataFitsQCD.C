...
#include "Common.h"
void PlotDataFitsQCD(int iCAT=0, 
		     int iORDER=3,
		     TString variable = "mbb_reg",
		     bool blind=false,
		     bool BERN=false,
		     double ymin_ratio = 0.9,
		     double ymax_ratio = 1.11) {
  ...
  TString CutsBlind = Cuts + "&&(mbb_reg<100||mbb_reg>150)";
  ...
  {  
    ...
    //    RooFitResult * fitRes = CHV.fitTo(data,SumW2Error(kTRUE),Save(),Range("low,high"),SplitRange(kFALSE));
    RooFitResult * fitRes = CHV.fitTo(data,SumW2Error(kTRUE),Save());
    cout << endl;
    fitRes->Print();
    RooAbsReal* intOfFunc = CHV.createIntegral(mbb);
    double integral = 0;
    double norm = 0;
    ...
    for (int iBin = 1; iBin<=NbinsBkg; ++iBin) {
      ...
      mbb.setVal(xCenter);
      double xModel  = scale*CHV.getVal();
  ...
  if(BERN)
   ...
   {
    sprintf(str3,"Cheb. Pol. Ord: %d",pol_ord);   
   }
