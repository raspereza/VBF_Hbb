...
void CreateSignalPDF(int iCAT,
  ...
  for (unsigned int i=0; i<5; ++i) {
    ...
    TH1D * hist = new TH1D(nameHist,"",NbinsSig_wj,xmin_wj,xmax_wj);    
    treeWJets[sysName]->Draw("mbb_reg>>"+nameHist,"weight*("+cuts[iCAT]+")");
    mapNorm[sysName] = hist->GetSumOfWeights();
    delete hist;

    hist = new TH1D(nameHist,"",NbinsSig_wj,xmin_wj,xmax_wj);
    treeWJets[sysName]->Draw("mbb_reg>>"+nameHist,"weight");

    RooRealVar mbbx("mbb","mass(bb)",xmin_wj,xmax_wj);
  ...
  RooRealVar mean("mean_wj_"+names[iCAT],"mean",90,80,200);
  ...
  RooAddPdf signal_wj("wj_"+names[iCAT],"WJets",RooArgList(gaus_wj,BRN_wj),fsig);
...
void CreateWJetsTemplates_sys() {
  ...
  //TFile * fileOutput = new TFile("workspace_singleb_powheg/wj_singleb_shapes.root","recreate");
  fileOutput->cd("");
