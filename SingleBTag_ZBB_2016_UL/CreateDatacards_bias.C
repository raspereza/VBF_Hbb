...
void CreateDatacards_bias(bool TF = false) {
  ...
  TFile * file = new TFile("rootshape/data_singleb_shapes"+suffix+".root");
  ...
  TH1F * wj_jec_up = (TH1F*)fs->Get("ZJets_JEC_up");
  TH1F * wj_jec_dn = (TH1F*)fs->Get("ZJets_JEC_dn");
  TH1F * wj_jer_up = (TH1F*)fs->Get("ZJets_JER_up");
  TH1F * wj_jer_dn = (TH1F*)fs->Get("ZJets_JER_dn");
  TH1F * wj_btg_up = (TH1F*)fs->Get("ZJets_BTG_up");
  TH1F * wj_btg_dn = (TH1F*)fs->Get("ZJets_BTG_dn");
  TH1F * wj_pu_up = (TH1F*)fs->Get("ZJets_PU_up");
  TH1F * wj_pu_dn = (TH1F*)fs->Get("ZJets_PU_dn");
  TH1F * vbfh_qgl = (TH1F*)fs->Get("VBF_Hbb_QGL_up");
  TH1F * ggh_qgl = (TH1F*)fs->Get("ggF_Hbb_QGL_up");
  TH1F * tt_qgl = (TH1F*)fs->Get("tt_QGL_up");
  TH1F * wj_qgl = (TH1F*)fs->Get("ZJets_QGL_up");
  TH1F * wj_isr_up = (TH1F*)fs->Get("ZJets_ISR_up");
  TH1F * wj_isr_dn = (TH1F*)fs->Get("ZJets_ISR_dn");
  TH1F * wj_fsr_up = (TH1F*)fs->Get("ZJets_FSR_up");
  TH1F * wj_fsr_dn = (TH1F*)fs->Get("ZJets_FSR_dn");
  TH1F * wj_puid_up = (TH1F*)fs->Get("ZJets_PUID_up");
  TH1F * wj_puid_dn = (TH1F*)fs->Get("ZJets_PUID_dn");
  TH1F * wj_trig_up = (TH1F*)fs->Get("ZJets_OnlineBTG_up");
  TH1F * wj_trig_dn = (TH1F*)fs->Get("ZJets_OnlineBTG_dn");
  ...
  for (int i=0; i<4; ++i) {
    ...
    ostringstream str;
    str << "datacards/datacards_singleb_" << names[i] << "_amcnlo_breg_with_sys.txt";
    string nn = str.str();
    ...
    textfile << "shapes          zj_bias         *   SingleB_ZBB_2016_UL/rootshape/zj_singleb_shapes.root  w:zj_$CHANNEL" << endl;
    textfile << "----------------------------------------------------------------------------------------------------" << endl;
    ...
    textfile << "process      ggH_hbb  qqH_hbb   qcd   tt   wj    zj   zj_bias" << endl;
    textfile << "process            1        2     3    4    5     0    6 " << endl;
    textfile << "rate               1        1     1    1    1     1    1 " << endl;
    textfile << "----------------------------------------------------------------------------------------------------" << endl;
    textfile << "CMS_BR_hbb     lnN	1.007/0.994	1.007/0.994	-      -     -     -     1.007/0.994     1.007/0.994" << endl;
    textfile << "----------------------------------------------------------------------------------------------------" << endl;

    textfile << "lumi_13TeV_BCC_2016  lnN  1.002    1.002    -    1.002   1.002   1.002   1.002" << endl;
    textfile << "lumi_13TeV_LS_2016   lnN  1.002    1.002    -    1.002   1.002   1.002   1.002" << endl;
    textfile << "lumi_13TeV_XY_2016   lnN  1.020    1.020    -    1.020   1.020   1.020   1.020" << endl;
    textfile << "----------------------------------------------------------------------------------------------------" << endl;
    textfile << fixed << setprecision(3)<< "CMS_BTag_2016       lnN     " << (1 + ggh_btg_up->GetBinContent(i+1)) << "/" <<  (1 + ggh_btg_dn->GetBinContent(i+1)) << "  " << (1 + vbfh_btg_up->GetBinContent(i+1)) << "/" <<  (1 + vbfh_btg_dn->GetBinContent(i+1))    << "        -       " <<   (1 + tt_btg_up->GetBinContent(i+1)) << "/" <<  (1 + tt_btg_dn->GetBinContent(i+1)) << "      " << (1 + wj_btg_up->GetBinContent(i+1)) << "/" <<  (1 + wj_btg_dn->GetBinContent(i+1)) << "   " << (1 + wj_btg_up->GetBinContent(i+1)) << "/" <<  (1 + wj_btg_dn->GetBinContent(i+1)) << "   " << endl;
    textfile << fixed << setprecision(3)<< "CMS_PUweight_2016   lnN     " << (1 + ggh_pu_up->GetBinContent(i+1)) << "/" <<  (1 + ggh_pu_dn->GetBinContent(i+1)) << "  " << (1 + vbfh_pu_up->GetBinContent(i+1)) << "/" <<  (1 + vbfh_pu_dn->GetBinContent(i+1))    << "        -       " <<   (1 + tt_pu_up->GetBinContent(i+1)) << "/" <<  (1 + tt_pu_dn->GetBinContent(i+1)) << "      " << (1 + wj_pu_up->GetBinContent(i+1)) << "/" <<  (1 + wj_pu_dn->GetBinContent(i+1)) << "   " << (1 + wj_pu_up->GetBinContent(i+1)) << "/" <<  (1 + wj_pu_dn->GetBinContent(i+1)) << "   "  <<  endl;
    textfile << fixed << setprecision(3)<< "CMS_QGLweight_2016  lnN     " << (1 + ggh_qgl->GetBinContent(i+1)) <<  "  " << (1 + vbfh_qgl->GetBinContent(i+1)) << "        -       " <<   (1 + tt_qgl->GetBinContent(i+1)) << "      " << (1 + wj_qgl->GetBinContent(i+1))  << "   " << (1 + wj_qgl->GetBinContent(i+1))  << "   "  << endl;
    textfile << fixed << setprecision(3)<< "CMS_PS_ISR_2016     lnN     " << (1 + ggh_isr_up->GetBinContent(i+1)) << "/" <<  (1 + ggh_isr_dn->GetBinContent(i+1)) << "  " << (1 + vbfh_isr_up->GetBinContent(i+1)) << "/" <<  (1 + vbfh_isr_dn->GetBinContent(i+1))    << "        -       " <<   (1 + tt_isr_up->GetBinContent(i+1)) << "/" <<  (1 + tt_isr_dn->GetBinContent(i+1)) << "      " << (1 + wj_isr_up->GetBinContent(i+1)) << "/" <<  (1 + wj_isr_dn->GetBinContent(i+1))  << "   " << endl;
    textfile << fixed << setprecision(3)<< "CMS_PS_FSR_2016     lnN     " << (1 + ggh_fsr_up->GetBinContent(i+1)) << "/" <<  (1 + ggh_fsr_dn->GetBinContent(i+1)) << "  " << (1 + vbfh_fsr_up->GetBinContent(i+1)) << "/" <<  (1 + vbfh_fsr_dn->GetBinContent(i+1))    << "        -       " <<   (1 + tt_fsr_up->GetBinContent(i+1)) << "/" <<  (1 + tt_fsr_dn->GetBinContent(i+1)) << "      " << (1 + wj_fsr_up->GetBinContent(i+1)) << "/" <<  (1 + wj_fsr_dn->GetBinContent(i+1)) << "   " << (1 + wj_fsr_up->GetBinContent(i+1)) << "/" <<  (1 + wj_fsr_dn->GetBinContent(i+1)) << "   " <<  endl;
    textfile << fixed << setprecision(3)<< "CMS_Trig_2016     lnN     " << (1 + ggh_trig_up->GetBinContent(i+1)) << "/" <<  (1 + ggh_trig_dn->GetBinContent(i+1)) << "  " << (1 + vbfh_trig_up->GetBinContent(i+1)) << "/" <<  (1 + vbfh_trig_dn->GetBinContent(i+1))    << "        -       " <<   (1 + tt_trig_up->GetBinContent(i+1)) << "/" <<  (1 + tt_trig_dn->GetBinContent(i+1)) << "      " << (1 + wj_trig_up->GetBinContent(i+1)) << "/" <<  (1 + wj_trig_dn->GetBinContent(i+1)) << "   " << (1 + wj_trig_up->GetBinContent(i+1)) << "/" <<  (1 + wj_trig_dn->GetBinContent(i+1)) << "   " << (1 + ggh_trig_up->GetBinContent(i+1)) << "/" <<  (1 + ggh_trig_dn->GetBinContent(i+1)) << "  " << (1 + vbfh_trig_up->GetBinContent(i+1)) << "/" <<  (1 + vbfh_trig_dn->GetBinContent(i+1))  << endl;
    textfile << fixed << setprecision(3)<< "CMS_PileUpID_2016     lnN     " << (1 + ggh_puid_up->GetBinContent(i+1)) << "/" <<  (1 + ggh_puid_dn->GetBinContent(i+1)) << "  " << (1 + vbfh_puid_up->GetBinContent(i+1)) << "/" <<  (1 + vbfh_puid_dn->GetBinContent(i+1))    << "        -       " <<   (1 + tt_puid_up->GetBinContent(i+1)) << "/" <<  (1 + tt_puid_dn->GetBinContent(i+1)) << "      " << (1 + wj_puid_up->GetBinContent(i+1)) << "/" <<  (1 + wj_puid_dn->GetBinContent(i+1)) << "   " << (1 + wj_puid_up->GetBinContent(i+1)) << "/" <<  (1 + wj_puid_dn->GetBinContent(i+1)) << "   " << (1 + ggh_puid_up->GetBinContent(i+1)) << "/" <<  (1 + ggh_puid_dn->GetBinContent(i+1)) << "  " << (1 + vbfh_puid_up->GetBinContent(i+1)) << "/" <<  (1 + vbfh_puid_dn->GetBinContent(i+1))  << endl;
