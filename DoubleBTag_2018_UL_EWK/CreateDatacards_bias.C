...
void CreateDatacards_bias(bool freeZnorm = true,
			  TString folder = "DoubleBTag_2018_UL_EWK"){
  ...
  TFile  *fs = new TFile("VBFHToBB_2018_UL_uncertainties_merged_DoubleB.root");
  ...
  for (int i=0; i<6; ++i) {
    ...
    //    RooRealVar * rate_ggH_var = wSig->var("norm_Nom_ggH_"+names[i]);
    //    RooRealVar * rate_qqH_var = wSig->var("norm_Nom_qqH_"+names[i]);
    ...
    ostringstream str;
    str << "datacards/datacards_doubleb_" << names[i] << "_EWK.txt";
    string nn = str.str();
    ...
    textfile << "kmax *" << endl;
    textfile << "----------------------------------------------------------------------------------------------------" << endl;
    textfile << "shapes          qcd             *   " << folder << "/root_shape/data_doubleb_shapes"+suffix+".root       w:qcd_$CHANNEL" << endl;
    textfile << "shapes          data_obs        *   " << folder << "/root_shape/data_doubleb_shapes.root       w:data_$CHANNEL" << endl;
    textfile << "shapes          qqH_hbb         *   " << folder << "/root_shape/signal_doubleb_shapes.root   w:qqH_$CHANNEL" << endl;
    textfile << "shapes          ggH_hbb         *   " << folder << "/root_shape/signal_doubleb_shapes.root   w:ggH_$CHANNEL" << endl;
    textfile << "shapes          qqH_hbb_bias    *   " << folder << "/root_shape/signal_doubleb_shapes.root   w:qqH_$CHANNEL" << endl;
    textfile << "shapes          ggH_hbb_bias    *   " << folder << "/root_shape/signal_doubleb_shapes.root   w:ggH_$CHANNEL" << endl;
    textfile << "shapes          zj              *   " << folder << "/root_shape/zj_doubleb_shapes.root  w:zj_$CHANNEL" << endl;
    textfile << "----------------------------------------------------------------------------------------------------" << endl;
    ...
    textfile << "lumi_13TeV_BCC_2018  lnN  1.002    1.002    -    1.002   1.002    1.002" << endl;
    textfile << "lumi_13TeV_LS_2018   lnN  1.002    1.002    -    1.002   1.002    1.002" << endl;
    textfile << "lumi_13TeV_XY_2018   lnN  1.020    1.020    -    1.020   1.020    1.020" << endl;
    textfile << "----------------------------------------------------------------------------------------------------" << endl;
    //Offline BTAG
    textfile << fixed << setprecision(3)<< "CMS_BTag_2018       lnN     " << (1 + ggh_btg_up->GetBinContent(i+1)) << "/" <<  (1 + ggh_btg_dn->GetBinContent(i+1)) << "  " << (1 + vbfh_btg_up->GetBinContent(i+1)) << "/" <<  (1 + vbfh_btg_dn->GetBinContent(i+1))    << "        -       " << (1 + zj_btg_up->GetBinContent(i+1)) << "/" <<  (1 + zj_btg_dn->GetBinContent(i+1)) << "   " << (1 + ggh_btg_up->GetBinContent(i+1)) << "/" <<  (1 + ggh_btg_dn->GetBinContent(i+1)) << "  " << (1 + vbfh_btg_up->GetBinContent(i+1)) << "/" <<  (1 + vbfh_btg_dn->GetBinContent(i+1))   << endl;
    ...
    //    textfile << fixed << setprecision(3)<< "CMS_QGLweight_2018  lnN     " << (1 + ggh_qgl->GetBinContent(i+1)) <<  "  " << (1 + vbfh_qgl->GetBinContent(i+1)) << "        -       "  << (1 + zj_qgl->GetBinContent(i+1))  << "   "  << (1 + ggh_qgl->GetBinContent(i+1)) <<  "  " << (1 + vbfh_qgl->GetBinContent(i+1)) << endl;
